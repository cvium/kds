...
public class WeierstrassSolver implements Solver<Complex> {
    ...
    public static Complex[] complexArray(double... a) {
        ...
        for (int i = a.length - 1; i > -1; i--) {
            ca[i] = Complex.valueOf(a[a.length - 1 - i], 0);
        }
        return ca;
    }
     ...
     * The array should have the highest order coefficient first.
     *
     * @param  da a variable number of Complex polynomial coefficients.
     * @return an array of the Complex roots of the polynomial.
     */
    public Complex[] findRoots(double[] da, double t) {
        Complex[] ca = complexArray(da);
        ...
        while (true) {
            for (int i = 0; i < a0.length; i++) {
                ...
                a1[i] = a0[i].minus(WeierstrassSolver.
